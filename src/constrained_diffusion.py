...
def constrained_diffusion_decomposition(
    ...
    min_scale=1,
    mode='log', 
    log_scale_base=2.0,       # Renamed parameter
    linear_scale_step=None,  # New parameter
    e_rel=3e-2, 
    sm_mode='reflect', 
    up_sample=True, 
    constrained=True,
    ...
    Args:
        ...
        return_scales (bool): If True, the tuple of scales used is also returned. Defaults to False.

    Returns:
        tuple: By default, returns a tuple of `(results, residual)`.
               If `return_scales=True`, returns `(results, residual, scales)`.
    """
    ...
    scales = None
    if mode == 'log':
        # For log mode, num_channels is the primary control
        if num_channels is None:
            print("Automatically determining num_channels for log mode.")
            # Use change of base formula: log_b(x) = log(x) / log(b)
            effective_num_channels = int(np.floor(np.log(effective_max_scale / effective_min_scale) / np.log(log_scale_base)))
        ...
        # np.logspace start/stop are powers, so we need log_base(scale)
        start_power = np.log(effective_min_scale) / np.log(log_scale_base)
        stop_power = np.log(effective_max_scale) / np.log(log_scale_base)
        scales = np.logspace(start_power, stop_power, num=effective_num_channels, base=log_scale_base)
    
    elif mode == 'lin':
        # For lin mode, linear_scale_step takes precedence
        if linear_scale_step is not None:
            print(f"Generating linear scales with a step of {linear_scale_step}.")
            scales = np.arange(effective_min_scale, effective_max_scale + linear_scale_step, linear_scale_step)
        else:
            # Fall back to num_channels if step is not provided
            if num_channels is None:
            ...
            if effective_num_channels < 1:
                raise ValueError(f"Number of channels must be at least 1. Calculated value was {effective_num_channels}.")
            scales = np.linspace(effective_min_scale, effective_max_scale, num=effective_num_channels)

    # Final validation on generated scales
    ...
    if up_sample:
        switch_scale = 5.0
        zoom_factor = 4
        scales_small = scales[scales <= switch_scale]
        scales_large = scales[scales > switch_scale]
        final_results = []
        current_data = data
        final_residual = data
        ...
        if len(scales_small) > 0:
            ...
            final_results.extend(results_small)
        ...
        if len(scales_large) > 0:
            ...
            results_large, final_residual = constrained_multiscale_decomposition(
                current_data, scales_large, **core_kwargs)
            final_results.extend(results_large)
        
        if return_scales:
            return final_results, final_residual, scales
        else:
            return final_results, final_residual
    else:
        print(f'\n--- Performing standard fixed-grid decomposition for all scales ---')
        results, residual = constrained_multiscale_decomposition(data, scales, **core_kwargs)
